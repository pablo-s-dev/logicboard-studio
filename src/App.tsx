  ...
  return <div className="app" onClick={() => { if (context) setContext(null); if (projectMenuOpen) setProjectMenuOpen(false); if (projectSwitcherOpen) setProjectSwitcherOpen(false); }}>
    ...
    <main className={`workspace ${hasProject ? "" : "no-project"} ${activityView === null ? "activity-collapsed" : ""} ${collapsed.inspector ? "inspector-collapsed" : ""} ${collapsed.bottom ? "bottom-collapsed" : ""}`} style={workspaceStyle}>
      ...
      <section className="bottom-panel">
        <div className="bottom-tabs"><CollapseButton direction={collapsed.bottom ? "up" : "down"} title={collapsed.bottom ? t("bottom.expand") : t("bottom.collapse")} onClick={() => togglePane("bottom")} /><button className={bottomTab === "waveform" ? "active" : ""} onClick={() => setBottomTab("waveform")}><Activity size={14} />{t("bottom.sample")}</button><button className={bottomTab === "compilation" ? "active" : ""} onClick={() => setBottomTab("compilation")}><TerminalSquare size={14} />{t("bottom.compilation")}</button><button className={bottomTab === "problems" ? "active" : ""} onClick={() => setBottomTab("problems")}><Info size={14} />{t("bottom.problems")} <i>{problems.length}</i></button></div>
