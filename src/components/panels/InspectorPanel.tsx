  ...
  return <aside className="inspector">
    <div className="inspector-tabs">
      ...
      {!collapsed && <>
        ...
        <button className={view === "mapper" ? "active" : ""} onClick={() => onView("mapper")}>{t("inspector.mapper")}</button>
      </>}
    </div>
    {collapsed ? <button className="panel-rail" title={t("inspector.expand")} onClick={onToggle}><Search size={15} /><span>{t("inspector.title")}</span></button> : <>
      <div className="search"><Search size={14} /><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder={view === "mapper" ? t("inspector.filter.mapper") : view === "assignments" ? t("inspector.filter.assignments") : t("inspector.filter.ports")} /></div>
      <div className="assignment-summary"><span>{view === "mapper" ? t("inspector.summary.mapper") : view === "assignments" ? t("inspector.summary.assignments") : t("inspector.summary.ports")}</span><b>{view === "ports" ? ports.length : `${mappedCount} / ${board.endpoints.length}`}</b></div>
...
function AssignmentsList({ board, expandedAssignments, term, onClear }: {
  ...
  if (!rows.length) return <p className="empty-list">{t("inspector.none.assignments")}</p>;
  return <>{rows.map((assignment) => {
    ...
    return <div className="assignment-row" key={`${assignment.assignmentId}-${assignment.endpointId}-${assignment.portId}`}>
      ...
      <div className="assignment-name"><b>{assignment.portId}</b><span>{assignment.endpointLabel} - {assignment.pin ? `PIN_${assignment.pin}` : t("inspector.pinUnknown")}</span></div>
      <button title={t("inspector.remove", { port: assignment.portId })} onClick={() => onClear(endpoint?.id ?? assignment.endpointId)}><Trash2 size={13} /></button>
    </div>;
  })}</>;
}
...
function PortsList({ board, ports, expandedAssignments, term }: {
  ...
  if (!rows.length) return <p className="empty-list">{t("inspector.none.ports")}</p>;
  return <>{rows.map((port) => {
    ...
    return <div className="assignment-row port-row" key={port.id}>
      ...
      <div className={endpoint ? "port-map mapped-text" : "port-map"}>{endpoint ? `${endpoint.label} ${endpoint.pin ? `PIN_${endpoint.pin}` : t("inspector.pinUnknown")}` : t("inspector.notMapped")}</div>
    </div>;
  })}</>;
}
...
function MapperList({ board, ports, assignments, expandedAssignments, term, onAssign, onClear }: {
  ...
  if (!groups.length) return <p className="empty-list">{t("inspector.none.endpoints")}</p>;
  return <>{groups.map((group) => {
    ...
    return <div className="mapper-group" key={group.id}>
      <div className="mapper-group-title"><b>{t(`board.group.${group.id}`)}</b><span className={`map-status ${status}`}>{t(`mapping.status.${status}`)}</span></div>
      {group.vectorName && <div className="mapper-row">
        <span>{t("inspector.vector")}</span>
        <select value={currentVector?.portId ?? ""} onChange={(event) => event.target.value && onAssign({ id: `vector:${group.id}:${event.target.value}`, kind: "vector", endpointId: group.id, portId: event.target.value })}>
          <option value="">{t("inspector.chooseVector")}</option>
          {vectorOptions.map((port) => <option key={port.name} value={port.name}>{port.name} - {port.type}</option>)}
        </select>
        <button title={t("inspector.clearVector")} onClick={() => onClear(group.id)}><Trash2 size={13} /></button>
      </div>}
      {[...group.children].sort(endpointSort).map((endpoint) => {
        ...
        return <div className="mapper-row child" key={endpoint.id}>
          ...
          <select value={current?.portId ?? ""} onChange={(event) => event.target.value && onAssign({ id: `granular:${endpoint.id}:${event.target.value}`, kind: "granular", endpointId: endpoint.id, portId: event.target.value })}>
            <option value="">{t("inspector.choosePort")}</option>
            {options.map((port) => <option key={port.id} value={port.id}>{port.id}</option>)}
          </select>
          <button title={t("inspector.clearPin")} onClick={() => onClear(endpoint.id)}><Trash2 size={13} /></button>
        </div>;
      })}
    </div>;
  })}</>;
